...
function init() {
   /* $('.validatebox-text').bind('blur', function() {
    ...
    $('#scheduleJobTable').datagrid({
        rownumbers: true,
        url: "/analysis-console/job/qeury/scheduleJobs",
        columns: column,
        method: 'POST',
        fit:true,
        loadMsg:'正在努力为您查找中……',
        ...
        toolbar:[
            ...
            },
            {
                ...
                handler:function(){
                    ...
                    var times = new Date();
                    times = times.getTime();
                    $.get("/analysis-console/job/pause-schedule-job", {'scheduleJobId':selectId,'timestamp':times}, function(){
                        init();
                    });
                }
            },
            {
                ...
                handler:function(){
                    ...
                    var times = new Date();
                    times = times.getTime();
                    $.get("/analysis-console/job/resume-schedule-job", {'scheduleJobId':selectId,'timestamp':times}, function(){
                        console.log("删除成功");
                        init();
                    });
                }
            },
            {
                ...
                handler:function(){
                    ...
                    var times = new Date();
                    times = times.getTime();
                    $.get("/analysis-console/job/run-once-schedule-job", {'scheduleJobId':selectId,'timestamp':times}, function(){
                        init();
                    });
                }
            },
            ...
            },
            {
                ...
                handler:function(){
                    var selectId =  $('#scheduleJobTable').datagrid('getSelected')['scheduleJobId'];
                    $.get("/analysis-console/job/input-schedule-job", {'scheduleJobId':selectId, 'keywords':'delUpdate'}, function(data){
                        if (data.code == 200) {
                            //打开窗体
                ...
                handler:function(){
                    $.messager.confirm('提示','确定删除？',function(r){
                        if (r){
                            ...
                            var times = new Date();
                            times = times.getTime();
                            $.get("/analysis-console/job/delete-schedule-job", {'scheduleJobId':selectId,'timestamp':times}, function(){
                                init();
                            });
                        }
                        return;
...
$("#saveScheduleForm #save,#cancel").click(function(){
    if ($(this).attr("icon") == 'icon-ok') {
        ...
        $.get("/analysis-console/job/validate/jobClass", {"jobClass": $("#jobClass").val()}, function(data){
            if (data.code != 200) {
        ...
        //校验jobName
        $.post("/analysis-console/job/validate/jobName", {"jobName": $("#jobName").val()}, function(data) {
            if (data.code == 200) {
        ...
        if ($("#saveScheduleForm").form('validate')) {
            $.ajax({
                type: "POST",
                url: "/analysis-console/job/save-schedule-job",
                dataType: "json",
