 ...
 * REDIS锁
 */
public class SNRedisLock implements IRedisLock{
     ...
     */
    public SNRedisLock(IRedisCache redisCache, String lockKey) {
        this(redisCache, lockKey, DEFAULT_ACQUIRE_TIMEOUT_MILLIS, DEFAULT_EXPIRY_TIME_MILLIS);
    }
     ...
     */
    public SNRedisLock(IRedisCache redisCache, String lockKey, int acquireTimeoutMillis) {
        this(redisCache, lockKey, acquireTimeoutMillis, DEFAULT_EXPIRY_TIME_MILLIS);
    }
     ...
     */
    public SNRedisLock(IRedisCache redisCache, String lockKey, int acquireTimeoutMillis, int expiryTimeMillis) {
        this(redisCache, lockKey, acquireTimeoutMillis, expiryTimeMillis, UUID.randomUUID());
    }
     ...
     */
    public SNRedisLock(IRedisCache redisCache, String lockKey, int acquireTimeoutMillis, int expiryTimeMillis, UUID uuid) {
     ...
     *             in case of thread interruption
     */
    protected synchronized boolean acquire(IRedisCache redisCache) throws InterruptedException {
        int timeout = acquiryTimeoutInMillis;
        while (timeout >= 0) {

            final Lock newLock = asLock(System.currentTimeMillis() + lockExpiryInMillis);
            if (redisCache.setnx(lockKeyPath, newLock.toString())) {
                this.lock = newLock;
                return true;
            }

            final String currentValueStr = redisCache.getString(lockKeyPath);
            final Lock currentLock = Lock.fromString(currentValueStr);
            if (currentLock.isExpiredOrMine(lockUUID)) {
                String oldValueStr = redisCache.getSet(lockKeyPath, newLock.toString());
    ...
    public boolean renew() throws InterruptedException {
        final Lock lock = Lock.fromString(redisCache.getString(lockKeyPath));
     ...
     * @param jedis
     */
    protected synchronized void release(IRedisCache redisCache) {
        if (isLocked()) {
        	redisCache.del(lockKeyPath);
            this.lock = null;
        }
    }
